jQuery(document).ready(function($){function copyToClipboard(){var $temp=$("<input>");$("body").append($temp);var heading=$("h1.entry-title").text();var link=$("link[rel='canonical']").attr("href");var text=heading+' '+link;$temp.val(text).select();document.execCommand("copy");$temp.remove()}
function copyToClipboardHandle(){copyToClipboard();$(this).addClass("copy-to-clipboard-active")}
if($.cookie("PHPSESSID")!==undefined){$("#copy-to-clipboard-wrapper").show();$("#copy-to-clipboard").on("click",copyToClipboardHandle)}
console.log("Start");(function($){"use strict";var dh;var faded=!1;dh=$('.entry-content').offset()&&$('.entry-content').offset().top;if($(window).width()>1000&&$('body').hasClass('single'))
$(window).scroll(function(){var offset=$(window).scrollTop();if(offset>270&&faded!=!0){setTimeout(function(){$('.adslot_1 #center').fadeOut(400,function(){$('.adslot_1 #center').css({'position':'static'}).fadeIn()})},300)
faded=!0;return}})
var $musthead=$('#musthead'),$dynamicHeader=$('#dynamic-header'),timer1,timer2,$dynamicNav=$('.dynamic-nav'),$toptodayWrap=$('.toptoday-wrap');if($('#wpadminbar').length){$musthead.addClass('header-wpadminbar')}
$(window).on('scroll',function(){if($(window).scrollTop()>350){$dynamicHeader.find('.logotype').removeClass('active').next().addClass('active')}else{$dynamicHeader.find('.share').removeClass('active').prev().addClass('active')}});$(window).on('scroll',function(){if($(window).scrollTop()>0){$musthead.addClass('thin');$('body').addClass('header-is-thin')}else{$musthead.removeClass('thin');$('body').removeClass('header-is-thin')}});$("header nav > ul > li").hover(function(){openSubmenu(this);var mleft=-((120-$(this).innerWidth())/2);$(this).find("ul").css("margin-left",mleft)},function(){clearTimeout(timer1),$(this).removeClass("sub-menu-open")});$(".dropdown-toggle").on("click",function(event){event.stopPropagation();var $dropdown=$(this).siblings(".dropdown-menu, .search1-form");0!==$dropdown.length&&($dropdown.hasClass("open")?d():(d(),$dropdown.addClass("open"),$("#content, .toggle-menu").bind("click",d)))});$(".toggle-news a").on("click",function(event){event.preventDefault();var menuIsOpen=$("body").hasClass("news-visible");menuIsOpen?($("body").removeClass("news-visible"),$(window).scrollTop(window.scrollTopBeforeMenuShow)):(window.scrollTopBeforeMenuShow=parseInt($(window).scrollTop()),$("body").addClass("news-visible"),$(window).scrollTop(0)),$(this).toggleClass("fa-newspaper-o fa-times");$('#news .toggle-featured-articles').click()});$(".toggle-menu a").on("click",function(event){event.preventDefault();var menuIsOpen=$("body").hasClass("menu-visible");menuIsOpen?($("body").removeClass("menu-visible"),$(window).scrollTop(window.scrollTopBeforeMenuShow)):(window.scrollTopBeforeMenuShow=parseInt($(window).scrollTop()),$("body").addClass("menu-visible"),$(window).scrollTop(0)),$(this).toggleClass("fa-bars fa-times")});$("#menu .toggle").on("click",function(event){event.preventDefault(),$(this).parents("ul.topics li").toggleClass("expanded")});if($toptodayWrap[0]===$dynamicNav.next()[0]){var dynamicInterval=5000;$(window).on('scroll',function(){if($(window).scrollTop()>0){clearInterval(timer2);timer2=setInterval(function(){if($dynamicNav.hasClass('active')){$dynamicNav.removeClass('active').parent().addClass('ovh');$toptodayWrap.addClass('active')}else{$dynamicNav.addClass('active');$toptodayWrap.removeClass('active');setTimeout(function(){$dynamicNav.parent().removeClass('ovh')},1000)}},dynamicInterval)}else{clearInterval(timer2);$dynamicNav.addClass('active');$toptodayWrap.removeClass('active');setTimeout(function(){$dynamicNav.parent().removeClass('ovh')},1000)}})}
$(window).scroll();function openSubmenu(b){timer1=setTimeout(function(){$(b).addClass("sub-menu-open")},150)};function d(b){$(".dropdown-menu.open, .search1-form.open").removeClass("open"),$("#content, .toggle-menu").unbind("click",d)};$('.notification  .close').click(function(){$('.notification').remove();$.cookie('notification_removed',1)})
if(!$.cookie('notification')){$.cookie('notification',1,{expires:7,path:'/'})}else{var notification=parseInt($.cookie('notification'));$.cookie('notification',notification+1,{expires:7,path:'/'})}
if($(window).width()>1000)
if($.cookie('notification')<2&&!$.cookie('notification_removed')){$('.notification').show()}})(jQuery)
...
return!1}
function testDOMProps(props,obj,elem){for(var i in props){var item=obj[props[i]];if(item!==undefined){if(elem===!1)
return props[i];if(is(item,'function')){return item.bind(elem||obj)}
return item}}
return!1}
...
function Spinner(o){if(typeof this=='undefined')
...
return el},opacity:function(el,i,val){if(i<el.childNodes.length)
...
function bindTransitionEnd($el){var elData=$el.data();if(elData.tEnd)
return;var el=$el[0],transitionEndEvent={WebkitTransition:'webkitTransitionEnd',MozTransition:'transitionend',OTransition:'oTransitionEnd otransitionend',msTransition:'MSTransitionEnd',transition:'transitionend'};addEvent(el,transitionEndEvent[Modernizr.prefixed('transition')],function(e){elData.tProp&&e.propertyName.match(elData.tProp)&&elData.onEndFn()});elData.tEnd=!0}
function afterTransition($el,property,fn,time){var ok,elData=$el.data();if(elData){elData.onEndFn=function(){if(ok)
return;ok=!0;clearTimeout(elData.tT);fn()};elData.tProp=property;clearTimeout(elData.tT);elData.tT=setTimeout(function(){elData.onEndFn()},time*1.5);bindTransitionEnd($el)}}
...
function findVideoId(href,forceVideo){if(typeof href!=='string')
return href;href=parseHref(href);var id,type;if(href.host.match(/youtube\.com/)&&href.search){id=href.search.split('v=')[1];if(id){var ampersandPosition=id.indexOf('&');if(ampersandPosition!==-1){id=id.substring(0,ampersandPosition)}
...
function getIndexFromHash(hash,data,ok,startindex){if(!ok)
return!1;if(!isNaN(hash))
return hash-(startindex?0:1);var index;for(var _i=0,_l=data.length;_i<_l;_i++){var dataFrame=data[_i];if(dataFrame.id===hash){index=_i;break}}
return index}
function smartClick($el,fn,_options){_options=_options||{};$el.each(function(){var $this=$(this),thisData=$this.data(),startEvent;if(thisData.clickOn)
return;thisData.clickOn=!0;$.extend(touch($this,{onStart:function(e){startEvent=e;(_options.onStart||noop).call(this,e)},onMove:_options.onMove||noop,onTouchEnd:_options.onTouchEnd||noop,onEnd:function(result){if(result.moved)
return;fn.call(this,startEvent)}}),{noMove:!0})})}
...
function getRatio(_ratio){if(!_ratio)
return;var ratio=+_ratio;if(!isNaN(ratio)){return ratio}else{ratio=_ratio.split('/');return+ratio[0]/+ratio[1]||undefined}}
function addEvent(el,e,fn,bool){if(!e)
return;el.addEventListener?el.addEventListener(e,fn,!!bool):el.attachEvent('on'+e,fn)}
...
function fade($el1,$el2,$frames,options,fadeStack,chain){var chainedFLAG=typeof chain!=='undefined';if(!chainedFLAG){fadeStack.push(arguments);Array.prototype.push.call(arguments,fadeStack.length);if(fadeStack.length>1)
return}
$el1=$el1||$($el1);$el2=$el2||$($el2);var _$el1=$el1[0],_$el2=$el2[0],crossfadeFLAG=options.method==='crossfade',onEndFn=function(){if(!onEndFn.done){onEndFn.done=!0;var args=(chainedFLAG||fadeStack.shift())&&fadeStack.shift();args&&fade.apply(this,args);(options.onEnd||noop)(!!args)}},time=options.time/(chain||1);$frames.removeClass(fadeRearClass+' '+fadeFrontClass);$el1.stop().addClass(fadeRearClass);$el2.stop().addClass(fadeFrontClass);crossfadeFLAG&&_$el2&&$el1.fadeTo(0,0);$el1.fadeTo(crossfadeFLAG?time:0,1,crossfadeFLAG&&onEndFn);$el2.fadeTo(time,0,onEndFn);(_$el1&&crossfadeFLAG)||_$el2||onEndFn()}
...
function touch($el,options){var el=$el[0],tail={},touchEnabledFLAG,startEvent,$target,controlTouch,touchFLAG,targetIsSelectFLAG,targetIsLinkFlag,tolerance,moved;function onStart(e){$target=$(e.target);tail.checked=targetIsSelectFLAG=targetIsLinkFlag=moved=!1;if(touchEnabledFLAG||tail.flow||(e.touches&&e.touches.length>1)||e.which>1||(lastEvent&&lastEvent.type!==e.type&&preventEvent)||(targetIsSelectFLAG=options.select&&$target.is(options.select,el)))
return targetIsSelectFLAG;touchFLAG=e.type==='touchstart';targetIsLinkFlag=$target.is('a, a *',el);controlTouch=tail.control;tolerance=(tail.noMove||tail.noSwipe||controlTouch)?16:!tail.snap?4:0;extendEvent(e);startEvent=lastEvent=e;moveEventType=e.type.replace(/down|start/,'move').replace(/Down/,'Move');(options.onStart||noop).call(el,e,{control:controlTouch,$target:$target});touchEnabledFLAG=tail.flow=!0;if(!touchFLAG||tail.go)
stopEvent(e)}
...
return;e&&stopEvent(e);preventEvent=!0;clearTimeout(preventEventTimeout);preventEventTimeout=setTimeout(function(){preventEvent=!1},1000);(options.onEnd||noop).call(el,{moved:moved,$target:$target,control:controlTouch,touch:touchFLAG,startEvent:startEvent,aborted:!e||e.type==='MSPointerCancel'})}
function onOtherStart(){if(tail.flow)
return;setTimeout(function(){tail.flow=!0},10)}
function onOtherEnd(){if(!tail.flow)
return;setTimeout(function(){tail.flow=!1},TOUCH_TIMEOUT)}
...
function onEnd(result){if(tail.noSwipe&&result.moved)
return;if(!tracked){startTracking(result.startEvent,!0)}
...
function appendElements(FLAG){if(FLAG===appendElements.f)
return;if(FLAG){$fotorama.html('').addClass(_fotoramaClass+' '+stampClass).append($wrap).before($style).before($anchor);addInstance(that)}else{$wrap.detach();$style.detach();$anchor.detach();$fotorama.html(fotoramaData.urtext).removeClass(stampClass);hideInstance(that)}
...
function setAutoplayInterval(interval){if(interval===!0)
interval='';opts.autoplay=Math.max(+interval||AUTOPLAY_INTERVAL,o_transitionDuration*1.5)}
...
spinnerStop();spinner=new Spinner($.extend(spinnerDefaults,opts.spinner,spinnerOverride,{direction:o_rtl?-1:1}));arrsUpdate();stageWheelUpdate();if(opts.autoplay)
setAutoplayInterval(opts.autoplay);o_thumbSide=numberFromMeasure(opts.thumbwidth)||THUMB_SIZE;o_thumbSide2=numberFromMeasure(opts.thumbheight)||THUMB_SIZE;stageWheelTail.ok=navWheelTail.ok=opts.trackpad&&!SLOW;stageNoMove();extendMeasures(opts,[measures]);o_navThumbs=o_nav==='thumbs';if(o_navThumbs){frameDraw(size,'navThumb');$navFrame=$navThumbFrame;navFrameKey=NAV_THUMB_FRAME_KEY;setStyle($style,$.Fotorama.jst.style({w:o_thumbSide,h:o_thumbSide2,b:opts.thumbborderwidth,m:opts.thumbmargin,s:stamp,q:!COMPAT}));$nav.addClass(navThumbsClass).removeClass(navDotsClass)}else if(o_nav==='dots'){frameDraw(size,'navDot');$navFrame=$navDotFrame;navFrameKey=NAV_DOT_FRAME_KEY;$nav.addClass(navDotsClass).removeClass(navThumbsClass)}else{o_nav=!1;$nav.removeClass(navThumbsClass+' '+navDotsClass)}
...
return $.each(indexes,function(i,index){if(rangeFLAG)
index=i;if(typeof index==='number'){var dataFrame=data[normalizeIndex(index)];if(dataFrame){var key='$'+type+'Frame',$frame=dataFrame[key];fn.call(this,i,index,dataFrame,$frame,key,$frame&&$frame.data())}}})}
function setMeasures(width,height,ratio,index){if(!measuresSetFLAG||(measuresSetFLAG==='*'&&index===startIndex)){width=measureIsValid(opts.width)||measureIsValid(width)||WIDTH;height=measureIsValid(opts.height)||measureIsValid(height)||HEIGHT;that.resize({width:width,ratio:opts.ratio||ratio||width/height},0,index!==startIndex&&'*')}}
function loadImg(indexes,type,specialMeasures,method,position,again){eachIndex(indexes,type,function(i,index,dataFrame,$frame,key,frameData){if(!$frame)
return;var fullFLAG=that.fullScreen&&dataFrame.full&&dataFrame.full!==dataFrame.img&&!frameData.$full&&type==='stage';if(frameData.$img&&!again&&!fullFLAG)
return;var img=new Image(),$img=$(img),imgData=$img.data();frameData[fullFLAG?'$full':'$img']=$img;var srcKey=type==='stage'?(fullFLAG?'full':'img'):'thumb',src=dataFrame[srcKey],dummy=fullFLAG?null:dataFrame[type==='stage'?'thumb':'img'];if(type==='navThumb')
$frame=frameData.$wrap;function triggerTriggerEvent(event){var _index=normalizeIndex(index);triggerEvent(event,{index:_index,src:src,frame:data[_index]})}
...
function frameDraw(indexes,type){eachIndex(indexes,type,function(i,index,dataFrame,$frame,key,frameData){if($frame)
return;$frame=dataFrame[key]=$wrap[key].clone();frameData=$frame.data();frameData.data=dataFrame;var frame=$frame[0];if(type==='stage'){if(dataFrame.html){$('<div class="'+htmlClass+'"></div>').append(dataFrame._html?$(dataFrame.html).removeAttr('id').html(dataFrame._html):dataFrame.html).appendTo($frame)}
...
function stageFramePosition(indexes){eachIndex(indexes,'stage',function(i,index,dataFrame,$frame,key,frameData){if(!$frame)
return;var normalizedIndex=normalizeIndex(index),method=dataFrame.fit||opts.fit,position=dataFrame.position||opts.position;frameData.eq=normalizedIndex;toDetach[STAGE_FRAME_KEY][normalizedIndex]=$frame.css($.extend({left:o_fade?0:getPosByIndex(index,measures.w,opts.margin,repositionIndex)},o_fade&&getDuration(0)));if(isDetached($frame[0])){$frame.appendTo($stageShaft);unloadVideo(dataFrame.$video)}
callFit(frameData.$img,measures,method,position);callFit(frameData.$full,measures,method,position)})}
function thumbsDraw(pos,loadFLAG){if(o_nav!=='thumbs'||isNaN(pos))
return;var leftLimit=-pos,rightLimit=-pos+measures.nw;$navThumbFrame.each(function(){var $this=$(this),thisData=$this.data(),eq=thisData.eq,getSpecialMeasures=function(){return{h:o_thumbSide2,w:thisData.w}},specialMeasures=getSpecialMeasures(),dataFrame=data[eq]||{},method=dataFrame.thumbfit||opts.thumbfit,position=dataFrame.thumbposition||opts.thumbposition;specialMeasures.w=thisData.w;if(thisData.l+thisData.w<leftLimit||thisData.l>rightLimit||callFit(thisData.$img,specialMeasures,method,position))
return;loadFLAG&&loadImg([eq],'navThumb',getSpecialMeasures,method,position)})}
...
return actual||$this.remove()&&!1}).sort(function(a,b){return $(a).data().eq-$(b).data().eq}).each(function(){if(!thumbsFLAG)
return;var $this=$(this),frameData=$this.data(),thumbwidth=Math.round(o_thumbSide2*frameData.data.thumbratio)||o_thumbSide;frameData.l=left;frameData.w=thumbwidth;$this.css({width:thumbwidth});left+=thumbwidth+opts.thumbmargin}));frameAppend[type]=!0}}
...
function extendMeasures(options,measuresArray){if(!options)
return;$.each(measuresArray,function(i,measures){if(!measures)
return;$.extend(measures,{width:options.width||measures.width,height:options.height,minwidth:options.minwidth,maxwidth:options.maxwidth,minheight:options.minheight,maxheight:options.maxheight,ratio:getRatio(options.ratio)})})}
...
function onTouchEnd(){if(!touchedFLAG)
return;if(!opts.stopautoplayontouch){releaseAutoplay();changeAutoplay()}
...
var _activeIndex=activeIndex;var frameData=activeFrame[STAGE_FRAME_KEY].data();changeAutoplay.w=waitFor(function(){return frameData.state||_activeIndex!==activeIndex},function(){changeAutoplay.t=setTimeout(function(){if(pausedAutoplayFLAG||_activeIndex!==activeIndex)
return;var _nextAutoplayIndex=nextAutoplayIndex,nextFrameData=data[_nextAutoplayIndex][STAGE_FRAME_KEY].data();changeAutoplay.w=waitFor(function(){return nextFrameData.state||_nextAutoplayIndex!==nextAutoplayIndex},function(){if(pausedAutoplayFLAG||_nextAutoplayIndex!==nextAutoplayIndex)
return;that.show(o_loop?getDirectionSign(!o_rtl):nextAutoplayIndex)})},opts.autoplay)})}
that.startAutoplay=function(interval){if(that.autoplay)
return this;pausedAutoplayFLAG=stoppedAutoplayFLAG=!1;setAutoplayInterval(interval||opts.autoplay);changeAutoplay();return this};that.stopAutoplay=function(){if(that.autoplay){pausedAutoplayFLAG=stoppedAutoplayFLAG=!0;changeAutoplay()}
return this};that.show=function(options){var index;if(typeof options!=='object'){index=options;options={}}else{index=options.index}
index=index==='>'?dirtyIndex+1:index==='<'?dirtyIndex-1:index==='<<'?0:index==='>>'?size-1:index;index=isNaN(index)?getIndexFromHash(index,data,!0):index;index=typeof index==='undefined'?activeIndex||0:index;that.activeIndex=activeIndex=edgeIndex(index);prevIndex=getPrevIndex(activeIndex);nextIndex=getNextIndex(activeIndex);nextAutoplayIndex=normalizeIndex(activeIndex+(o_rtl?-1:1));activeIndexes=[activeIndex,prevIndex,nextIndex];dirtyIndex=o_loop?index:activeIndex;var diffIndex=Math.abs(lastActiveIndex-dirtyIndex),time=getNumber(options.time,function(){return Math.min(o_transitionDuration*(1+(diffIndex-1)/12),o_transitionDuration*2)}),overPos=options.overPos;if(options.slow)
time*=10;var _activeFrame=activeFrame;that.activeFrame=activeFrame=data[activeIndex];var silent=_activeFrame===activeFrame&&!options.user;unloadVideo($videoPlaying,activeFrame.i!==data[normalizeIndex(repositionIndex)].i);frameDraw(activeIndexes,'stage');stageFramePosition(SLOW?[dirtyIndex]:[dirtyIndex,getPrevIndex(dirtyIndex),getNextIndex(dirtyIndex)]);updateTouchTails('go',!0);silent||triggerEvent('show',{user:options.user,time:time});pausedAutoplayFLAG=!0;var onEnd=that.show.onEnd=function(skipReposition){if(onEnd.ok)
return;onEnd.ok=!0;skipReposition||stageShaftReposition(!0);if(!silent){triggerEvent('showend',{user:options.user})}
...
arrsUpdate();if(o_nav){navUpdate();var guessIndex=limitIndex(activeIndex+minMaxLimit(dirtyIndex-lastActiveIndex,-1,1));slideNavShaft({time:time,coo:guessIndex!==activeIndex&&options.coo,guessIndex:typeof options.coo!=='undefined'?guessIndex:activeIndex,keep:silent});if(o_navThumbs)
...
function stageCursor(e){if(stageShaftTouchTail.flow)
return;var x=e?e.pageX:stageCursor.x,pointerFLAG=x&&!disableDirrection(getDirection(x))&&opts.click;if(stageCursor.p!==pointerFLAG&&$stage.toggleClass(pointerClass,pointerFLAG)){stageCursor.p=pointerFLAG;stageCursor.x=x}}
...
if(size){if(changeToRtl())
return;if($videoPlaying){unloadVideo($videoPlaying,!0)}
...
if(hwNeedLinks){var $linkArrow=$('<a id="prewbutton" href="#">&lt;</a><a id="nextbutton" href="#">&gt;</a>').prependTo('#slider');$('#nextbutton').click(function(){animSlide("next");return!1})
...
$('#slider-wrap').hover(function(){clearTimeout(slideTime);pause=!0},function(){pause=!1;rotator()});rotator()})}
if($('#slider-wrap').length>0){var loggedIn=$('body').hasClass('logged-in');$.get('/wp-content/themes/inforesist/sidebar/posts_data.json',{"_":$.now()},function(data){var res='';var j=0;var image='';$.each(data,function(i,val){if(j==0){if((i+3)>data.length)
return!0;res+='<div class="slide">'}
if(loggedIn){image='<img src="'+val.av+'" alt="author">'}else{image='<img data-lazy-src="'+val.av+'" alt="author" class="rocket-lazyload">'}
res+='<div class="icon"><div class="left"><div class="title"><a href="'+val.link+'"><span>'+val.title+'</span></a></div><div class="name"><span>'+val.author+'</span><div class="red"></div></div></div><div class="img">'+image+'</div></div>';j++;if(j==3){j=0;res+='</div>'}else if(i==data.length)
res+='</div>'});$('#slider-wrap').find('#slider').html(res);update_sidebar();if(!loggedIn){try{var lazyLoadInstance=new LazyLoad({elements_selector:"img[data-lazy-src],.rocket-lazyload",data_src:"lazy-src",class_loaded:"lazyloaded",class_loading:"lazyloading"})}catch(e){}}})}});$(document).ready(function(){$(".hider").click(function(){$("#hidden").slideToggle("slow");return!1})})})